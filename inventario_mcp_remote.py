...
Server HTTP MCP che può essere deployato su cloud e usato da Claude Desktop
"""
...
# Inizializza FastMCP con supporto HTTP
mcp = FastMCP("inventario-alimentare-remoto")
...
async def dettagli_alimento(id_alimento: int) -> str:
    ...
    try:
        async with db_pool.acquire() as conn:
            row = await conn.fetchrow("SELECT * FROM alimenti WHERE id = $1", id_alimento)
...
# Health check endpoint con FastAPI
app = FastAPI(
    title="MCP Server Inventario Remoto",
    description="Server MCP remoto per inventario alimentare - compatibile Claude Desktop",
    version="1.0.0"
)

# CORS per permettere connessioni remote
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
async def startup_event():
    ...
    logger.info(f"🚀 Server MCP Remoto avviato su porta {PORT}")

if __name__ == "__main__":
    import uvicorn
    
    # Per deployment su Render: avvia solo FastAPI con health check
    # mcp-remote si collegherà tramite HTTP
    uvicorn.run(
        app,
        host="0.0.0.0",
        port=PORT,
